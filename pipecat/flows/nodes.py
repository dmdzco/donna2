...
def build_opening_node(session_state: dict, flows_tools: dict) -> NodeConfig:
    ...
    return NodeConfig(
        ...
        task_messages=[
            {"role": "system", "content": senior_ctx},
            {"role": "system", "content": opening_task},
        ],
        functions=functions,
...
def build_winding_down_node(session_state: dict, flows_tools: dict) -> NodeConfig:
    ...
    return NodeConfig(
        ...
        task_messages=[
            {"role": "system", "content": senior_ctx},
            {"role": "system", "content": winding_task},
        ],
        functions=functions,
        context_strategy=ContextStrategyConfig(strategy=ContextStrategy.APPEND),
        respond_immediately=False,
...
def build_closing_node(session_state: dict) -> NodeConfig:
    ...
    return NodeConfig(
        name="closing",
        role_messages=[{"role": "system", "content": BASE_SYSTEM_PROMPT}],
        task_messages=[{"role": "system", "content": closing_task}],
        functions=[],
