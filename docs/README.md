...
| P0 | Prove manual call initiation reaches the Pipecat voice pipeline. | `routes/calls.js`, `pipecat/api/routes/voice.py`, `pipecat/main.py`, `pipecat/bot.py` | A caregiver/admin-triggered call from Node `/api/call` creates a Twilio call whose callbacks hit Pipecat `/voice/answer` and `/voice/status`, then the `/ws` pipeline starts with the expected senior context. Logs show only masked phone data. | `make deploy-dev`; trigger a dev call with a dummy/consenting pilot phone; verify Twilio callback URLs and Pipecat call metadata |
| P0 | Prove inbound known-senior and onboarding voice calls. | `pipecat/api/routes/voice.py`, `pipecat/bot.py`, `pipecat/flows/nodes.py`, `pipecat/services/prospects.py` | A known senior inbound call loads the senior profile and normal flow. An unrecognized caller uses the onboarding flow on the same Pipecat STT/LLM/TTS stack, creates or resumes a prospect, and does not deliver senior reminders. | Dev Twilio inbound test for one known test number and one unrecognized test number; verify prospect/senior DB state |
