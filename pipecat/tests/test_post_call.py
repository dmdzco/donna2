...
class TestPostCallProcessing:
    ...
    async def test_onboarding_post_call_flow(self, session_state):
        """Onboarding calls use the lighter prospect summary/update flow,
        without subscriber analysis, interest discovery, daily context, or
        snapshot rebuild."""
        ...
        with _base_patches() as mocks, \
            ...
            mock_summary.return_value = "Caller asked about care for their mom."
            ...
            # Should summarize and update prospect record
            mock_summary.assert_awaited_once()
            mock_update_prospect.assert_awaited_once_with(
                "prospect-001",
                {"caller_context": {"call_summary": "Caller asked about care for their mom."}},
            )
    ...
    async def test_call_metrics_data_structure(self, session_state):
        ...
        assert token_usage["prompt_tokens"] == 1500
        assert token_usage["completion_tokens"] == 800
        assert token_usage["cache_read_tokens"] == 250
